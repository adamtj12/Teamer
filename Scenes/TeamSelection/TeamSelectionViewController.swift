...
{
    func onPlayerUpdateReady()
}


class TeamSelectionViewController: UIViewController, TeamSelectionDisplayLogic, updatePlayer
{
    func onPlayerUpdateReady() {
        self.getTeamData()
    }
  ...
  func displayPlayerInformation(playerModel: TeamSelection.Something.TeamResponse)
  {
    ...
    self.addPlayerCollection.reloadData()
  }
...
extension TeamSelectionViewController : UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        if(section == 1){
            ...
            return (viewModel?.groupArrayUA.count)!
        }
        return 1
    }
}
...
extension TeamSelectionViewController : UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if(indexPath.section == 0){
            ...
            selectedPlayer = (viewModel?.groupArrayB.object(at: indexPath.row) as? TeamSelection.Something.PlayerModel)!
        }
        else {
            selectedPlayer = (viewModel?.groupArrayUA.object(at: indexPath.row) as? TeamSelection.Something.PlayerModel)!
        }
...
//        performSegue(withIdentifier: "playerInformationSegue", sender: nil)
    ...
    func setUpCollectionViewWithModel(model : TeamSelection.Something.TeamResponse, cell: PinEntryCellView, path: NSIndexPath)
    {
        ...
        if(path.section == 0){
            player =  model.groupArrayA.object(at: path.row) as! TeamSelection.Something.PlayerModel
        }else if(path.section == 1){
            player =  model.groupArrayB.object(at: path.row) as! TeamSelection.Something.PlayerModel
        }
        else{
            player =  model.groupArrayUA.object(at: path.row) as! TeamSelection.Something.PlayerModel
        }
    ...
    func updateDataWithEdit(player: EditPlayerDetails.Something.PlayerModel){
        print("SUCCESS")
        self.viewDidLoad()
    }
}
